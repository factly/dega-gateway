...
export class StatusComponent implements OnInit, OnDestroy {
    ...
    openDialogPopUp(statusDetails): void {
        const title = statusDetails.name;
        const config = {
            data: {
                message: `You are going to delete status with the title "${title}" ?`
            }
        };
        ...
        dialogRef.afterClosed().subscribe(selectedOption => {
            if (selectedOption.accept) {
                this.statusService.delete(statusDetails.id).subscribe(response => {
                    this.loadAll();
                });
            }
        });

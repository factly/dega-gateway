...
export class RoleComponent implements OnInit, OnDestroy {
    ...
    openDialogPopUp(roleDetails): void {
        const title = roleDetails.name;
        const config = {
            data: {
                message: `You are going to delete role with the title "${title}" ?`
            }
        };
        ...
        dialogRef.afterClosed().subscribe(selectedOption => {
            if (selectedOption.accept) {
                this.roleService.delete(roleDetails.id).subscribe(response => {
                    this.loadAll();
                });
            }
        });
